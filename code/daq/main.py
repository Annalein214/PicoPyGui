...
# ****** Measurement **********************************************************
    ...
    def prepareAdministration(self):
        ...
        st=timestring_humanReadable(self.startthread)
    ...
    def run(self):
        ...
        self.prepareAdministration()
        ok=self.setupScope()
        if not ok: return False # stop the run
        # setup varibales of this loop
        self.indisposedTimes=[] # save the values returned by runBlock, whatever they are
        self.measurementtime=[]
        # prepare varibles needed for analysis
        self.prepareAnalysis()
        # prepare loop
        self.endtime = time.time() # need this value for the first loop
        self.lastSaved = self.endtime # keep track of saving the data every hour
        ...
        while not self._threadIsStopped:
            ...
            self.measurementtime.append(endBlock-startBlock) # ns
            # ---- BLOCK END -----
            ...
            if not self.opts.test:
                ...
                for i in range(self.scope.NUM_CHANNELS):
                    waveforms=[]
                    for i in range(100):
                        wfm=np.random.randint(-10,10, self.nosamples)
            ...
            if self.endtime - self.lastSaved > (60*60):
                ...
                # TODO SAVE ALL
                self.lastSaved = self.endtime # this should move to saveall
                self.saveMeasurement=True# this should move to saveall

            # --- stop the loop ---
            print(self.endtime - self.startthread, self.measurementduration*60)
            if self.measurementduration!=0 and ((self.measurementduration*60) < (self.endtime - self.startthread)):
                # TODO SAVE ALL
    ...
    def analysis(self,data):
        ...
        for i in range(len(data)):
            ...
            if self.save_avg_std[channel]:
                ...
                self.avg[channel].extend(arrai)
                arrai=self.std(data[i])
                self.std[channel].extend(arrai)
            if self.save_fft[channel]:
                arrai=self.calcFft(data[i], channel)
                self.fft[channel].extend(arrai)
       
    def getWFM(self,dataX, channel):
        dx=np.array(dataX)
        if self.save_wfm_nbr[channel]>0:
            fakearray=np.arange(0,len(dx),1)
            index=np.random.choice(fakearray, size=self.save_wfm_nbr[channel])
            arrai=dx[index]
        else:
            arrai=dataX
        return arrai
    ...
    def average(self, dataX):
        arrai=np.mean(dataX, axis=1) # first axis: column, second axis: row
        return arrai

    def std(self, dataX):
        arrai=np.std(dataX, axis=1) # first axis: column, second axis: row
        return arrai
